...
export const claimReverseSubmarineSwap = async ({ address, feeRate = 1, swapInfo, privateKey, preimage, apiUrl, network: networkId }: ClaimReverseSubmarineSwapProps) => {
  ...
  const musig = new Musig(zkp, keyPair, randomBytes(SESSION_ID_BYTES), [
    boltzPublicKey,
    keyPair.publicKey,
  ]);
  ...
  musig.initializeSession(
    LiquidTaprootUtils.hashForWitnessV1(
      network,
      liquidClaimDetails,
      claimTx,
      0,
